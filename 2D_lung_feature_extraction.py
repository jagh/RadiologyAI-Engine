...
def extract_slices_dataframe(cts_folder, seg_folder, dataframe, sandbox='sandbox', task='SNF-lung', split='train'):
    """
    ...
    for row in range(metadata.shape[0]):
        ...
        axial_index = metadata['slice_index'][row]
        ...
        ct_slice_name = str(metadata['ct_file_name'][row]) + '-' + str(axial_index) + '_0000.nii.gz'
        seg_slice_name = str(metadata['ct_file_name'][row]) + '-' + str(axial_index) + '-GT_'+ task +'.nii.gz'
...
def pyRadiomics_feature_extraction(dataframe, sandbox, task='SNF-lung', split="test", testbed_name="testbed-FirstPaper", radiomics_set="bb"):
    """
    ...
    if split == 'test':
        metadata = metadata_full.query('split == "test"')
        axial_slices_folder = os.path.join(sandbox, "01_2D-axial_slices", task, str("imagesTs"))
        axial_labels_folder = os.path.join(sandbox, "01_2D-axial_slices", task, str("labelsTs"))
    else:
        metadata = metadata_full.query('split == "train"')
        axial_slices_folder = os.path.join(sandbox, "01_2D-axial_slices", task, str("imagesTr"))
        axial_labels_folder = os.path.join(sandbox, "01_2D-axial_slices", task, str("labelsTr"))
    ...
    filename = str(radiomics_folder+"/00_lesion_features-"+ radiomics_set + "-" + split +".csv")
    ...
    for row in range(metadata.shape[0]):
        ...
        ## Locating the CTs files
        ct_file_name = str(metadata['ct_file_name'][row] + "-" + str(metadata['slice_index'][row]) + "_0000.nii.gz")
        ct_file_path = os.path.join(axial_slices_folder, ct_file_name)
        print("+ ct_file_path: ", ct_file_path)


        ## Locating the segmentation files
        seg_file_name = str(metadata['ct_file_name'][row] + "-" + str(metadata['slice_index'][row]) + "-GT_SNF-lung.nii.gz")
